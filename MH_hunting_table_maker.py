...
df = pd.DataFrame(data={'Size':0,'Type':0,'Name':0,'Total Hunted':0,'Large Crown %':0,'Small Crown %':0},index=(0,1))
...
for file in input:
    ...
    for i in range(len(names)):
        s = pd.Series({'Size':0,'Type':0,'Name':names[i],'Total Hunted':0,'Large Crown %':0,'Small Crown %':0})
